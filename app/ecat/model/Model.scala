...
import Scalaz._
import shapeless._,ops.traversable.FromTraversable._, ops.record.Values,shapeless.syntax.std.traversable._, shapeless.contrib.scalaz._
import record._
...
object Schema {
  type Room = Record.`'number->Int, 'guestsCnt->Int, 'additionalGuestsCnt-> Int, 'twin->Boolean, 'bathroom->String, 'level-> Int, 'options-> Seq[String]`.T
  ...
  type Category = Record.`'id-> String, 'name->String, 'rooms-> Seq[Room], 'tariffs-> Seq[Tariff], 'prices-> Prices`.T
  type Hotel =  Record.`'id-> String, 'name-> String, 'checkInTime-> LocalTime, 'checkOutTime-> LocalTime, 'categories-> Seq[Category]`.T
}
...
object Category {
  ...
  def fromXml(n: Node, from: LocalDateTime, to: LocalDateTime): ValidationNel[String, Schema.Category] = {
    ...
    val catId =  n \@ "id"
    def name = n \@ "name"
    def rooms: ValidationNel[String, Seq[Schema.Room]] = Preprocessing.rooms((n \ "room").map(Room.fromXml))
    def tariffs: ValidationNel[String, Seq[Schema.Tariff]] = Preprocessing.tariffs(n \ "tarif" map (Tariff.fromXml), from, to)
  ...
  object Hotel {
    ...
    def fromXml(n: Node, from: LocalDateTime, to: LocalDateTime): ValidationNel[String, Seq[Schema.Hotel]] = {
      ...
      (n \ "hotel").map { hotelNode =>
        ...
        def cats:ValidationNel[String, Seq[Schema.Category]] = {
            (hotelNode \ "category")
